...
(() => {
  const boot = () => {
    ...
    menu.innerHTML = '<div class="thrn-auth-title">Welcome to THRN</div><div class="thrn-auth-copy">Your senior AI marketing consultant for growth, audits, and strategies.</div><button type="button" class="thrn-auth-signin">Sign in with Google</button><button type="button" class="thrn-auth-email">Sign in with email</button>';
    ...
    menu.querySelector('.thrn-auth-signin')?.addEventListener('click', () => {
      window.location.assign('/auth/google');
    });
    menu.querySelector('.thrn-auth-email')?.addEventListener('click', () => {
    ...
    fetch('/auth/me', { credentials: 'same-origin', cache: 'no-store' })
      ...
      .then((data) => {
        if (data && data.authenticated) {
          ...
          menu.querySelector('.thrn-auth-signout')?.addEventListener('click', () => {
